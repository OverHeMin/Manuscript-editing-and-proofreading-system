...
async function main() {
  ...
  const outputPath = path.join(
    outputDir,
    `gold-set-${document.gold_set_version.id}-v${document.gold_set_version.version_no}.jsonl`,
  );
  const jsonl = `${document.gold_set_version.items
...
function printHelp() {
  process.stdout.write(
    [
      "Usage: node scripts/harness/export-gold-set-jsonl.mjs --input <path> [--output-dir <path>]",
      "",
      "Reads a local governed gold-set JSON document and emits one JSON object per",
      "curated item into the bounded harness export directory. The input document must",
      "contain a published gold_set_version payload.",
      "",
      "Options:",
