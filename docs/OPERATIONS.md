...
- For machine-readable output, run `pnpm --filter @medical/api run recover:governed-orchestration -- --json`, or pair it with `-- --dry-run --json` for read-only backlog inspection output. `--actionable-only`, `--limit <n>`, repeatable `--module <module>`, and repeatable `--log-id <execution-log-id>` can be combined with the same dry-run JSON mode.
