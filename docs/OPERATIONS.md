...
- Phase 10N extends the same repo-owned recovery lane with bounded scope controls: repeat `--module <module>` and `--log-id <execution-log-id>` to narrow either replay or dry-run inspection to a specific subset of logs. These flags only reduce the candidate set; they do not override retry cooldowns, stale-running protection, business-completion requirements, or terminal orchestration failure.
- Phase 10O extends the same repo-owned recovery lane with bounded replay budgeting: add `--budget <n>` to replay only the next `n` eligible logs inside the current scope. This budget is optional, replay-only, and summary output reports the scoped eligible-versus-remaining backlog for that invocation.
- Phase 10P aligns budgeted replay with the existing recoverable-priority model already visible in dry-run categories: when `--budget <n>` is present, stale-running reclaim candidates are replayed before plain `recoverable_now` candidates inside the same scope. No-budget replay keeps its prior order.
- Phase 10R extends that same lane with a read-only replay preview: `recover:governed-orchestration -- --dry-run --budget <n>` shows the exact next bounded replay slice and preview metadata (`selected`, `eligible`, `remaining`, `budget`) before any durable mutation occurs.
- Phase 10S extends the same dry-run lane with normalized readiness metadata on each inspection item: `readiness=ready_now|waiting_retry_eligibility|waiting_running_timeout|not_recoverable`, plus `ready_at=<timestamp>` when blocked work has a concrete next-ready time.
- Phase 10T extends that same dry-run lane with a summary-level readiness rollup: the human-readable summary now appends `ready_now`, `waiting_retry_eligibility`, `waiting_running_timeout`, and the earliest `next_ready_at` derived from the same item-level readiness model.
- Phase 10U extends the same local CLI lane with additive JSON contract metadata: `--json` output now includes `report_kind`, `contract_version`, and `requested_options`, while preserving the current replay summary and dry-run report fields for fail-open consumers.
