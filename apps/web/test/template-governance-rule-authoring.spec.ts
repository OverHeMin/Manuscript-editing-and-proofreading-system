...
test("rule authoring form keeps parameter controls without guide copy", () => {
  const draft = createRuleAuthoringDraft("table");
  ...
  assert.match(markup, /置信策略/u);
  assert.match(markup, /关联知识条目/u);
  assert.match(markup, /data-rule-linked-knowledge="field"/u);
  assert.doesNotMatch(markup, /data-rule-parameter-guide="field"/u);
  assert.doesNotMatch(markup, /参数说明/u);
  assert.doesNotMatch(markup, /规则中心负责可执行判断/u);
  assert.doesNotMatch(markup, /知识条目只放证据、依据、范例或解释/u);
  assert.doesNotMatch(markup, /执行方式决定是自动改写还是只检查/u);
  assert.doesNotMatch(markup, /置信策略决定何时允许自动执行/u);
});
