...
export function applyTableEvidencePatch(input: {
    ...
    switch (operation.op) {
      ...
      case "confirm_invisible_chars":
        validateInvisibleCharIds(
          validateCellIds(confirmed.grid_cells, operation.cell_ids),
          operation.confirmed_invisible_char_ids,
        );
        break;
      case "confirm_special_symbols":
        validateSymbolRunIds(
          validateCellIds(confirmed.grid_cells, operation.cell_ids),
          operation.confirmed_symbol_run_ids,
        );
        break;
    }
  }
...
function validateSymbolRunIds(cells: TableEvidenceCellSnapshot[], runIds: string[]): void {
  const existingRunIds = new Set(cells.flatMap((cell) => cell.runs.map((run) => run.id)));
...
function recomputeCellText(cell: TableEvidenceCellSnapshot): TableEvidenceCellSnapshot {
  ...
  if (paragraphs.length === 0) {
    return {
      ...
      invisible_chars: cell.invisible_chars.map((entry) => ({ ...entry })),
      runs: cell.runs.map(cloneRun),
    };
  }
...
function cloneRun(run: TableEvidenceTextRun): TableEvidenceTextRun {
  return {
    ...run,
