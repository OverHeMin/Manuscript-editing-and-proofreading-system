...
import {
  ensureDeepSeekModuleRoutingConfiguration,
} from "../../src/database/scripts/seed-deepseek-module-routing.ts";
...
test("deepseek module routing seed creates the connection, models, and active module fallbacks idempotently", async () => {
  const harness = createHarness();

  await ensureDeepSeekModuleRoutingConfiguration({
    actorRole: "admin",
    actorId: "seed-script",
    ...
    connection: {
      ...
      apiKey: "sk-deepseek-seed-12345678",
    },
    modules: {
      screening: {
        primaryModelName: "deepseek-screening-primary",
        fallbackModelName: "deepseek-screening-fallback",
      },
      editing: {
        primaryModelName: "deepseek-editing-primary",
        fallbackModelName: "deepseek-editing-fallback",
      },
      proofreading: {
        primaryModelName: "deepseek-proofreading-primary",
        fallbackModelName: "deepseek-proofreading-fallback",
      },
    },
  });
  ...
  assert.equal(firstConnections[0]?.name, "DeepSeek Production");
  assert.equal(firstConnections[0]?.provider_kind, "deepseek");
