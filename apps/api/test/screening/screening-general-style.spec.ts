...
test("screening keeps general style findings visible without converting them into medical escalation signals", async () => {
  ...
  const screeningService = new ScreeningService({
      ...
      async runChecks() {
        return {
          ...
          quality_findings_summary: {
            ...
            representative_issue_ids: ["screening-style-1"],
          },
    ...
    createId: () => "job-screening-style-1",
  } as never);
  ...
  const result = await screeningService.run({
    ...
    actorRole: "screener",
    storageKey: "screening/manuscript-1/style-report.md",
    fileName: "style-report.md",
  });

  assert.equal(
    (
      result.job.payload?.qualityFindings as Array<{
        issue_type: string;
        module_scope: string;
      }>
    )?.[0]?.issue_type,
  ...
  assert.equal(
    Array.isArray(result.job.payload?.medicalReviewSignals),
    false,
  );
});
