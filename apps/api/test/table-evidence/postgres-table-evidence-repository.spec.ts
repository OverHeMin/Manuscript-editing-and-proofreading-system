...
import { PostgresTableEvidenceRepository } from "../../src/modules/table-evidence/index.ts";
import { withTemporaryDatabase } from "../database/support/postgres.ts";
import { runMigrateProcess } from "../database/support/migrate-process.ts";

test("postgres table evidence repository stores immutable revisions and target bindings", async () => {
  await withMigratedTableEvidenceClient(async (client) => {
    ...
    const sourceFile = await repository.findSourceFileById("file-1");
    ...
    assert.equal(sourceFile?.sha256, "sha256-file");
    assert.equal(asset?.title, "Table 1");
...
test("postgres table evidence repository keeps revision asset membership immutable", async () => {
  await withMigratedTableEvidenceClient(async (client) => {
    const repository = new PostgresTableEvidenceRepository({ client });
...
async function withMigratedTableEvidenceClient(
  ...
  await withTemporaryDatabase(async (databaseUrl) => {
    ...
    assert.equal(
      migrate.status,
      0,
      `Expected migrate to succeed for the temporary table evidence persistence database.\n${migrate.stdout}\n${migrate.stderr}`,
    );
...
async function seedTwoAssetHarness(
  repository: PostgresTableEvidenceRepository,
): Promise<void> {
  await repository.saveSourceFile({
    id: "file-1",
