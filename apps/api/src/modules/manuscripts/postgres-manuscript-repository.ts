...
export class PostgresManuscriptRepository implements ManuscriptRepository {
  ...
  async listRecent(limit: number): Promise<ManuscriptRecord[]> {
    const result = await this.dependencies.client.query<ManuscriptRow>(
      `
        select
          id,
    ...
    return result.rows.map((row) => mapManuscriptRow(row));
  }

  async archive(id: string, archivedAt: string): Promise<ManuscriptRecord | undefined> {
    const result = await this.dependencies.client.query<ManuscriptRow>(
      `
        update manuscripts
        set status = 'archived',
