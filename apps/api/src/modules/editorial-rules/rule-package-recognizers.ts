...
const PACKAGE_BLUEPRINTS: Record<RulePackageKind, RulePackageBlueprint> = {
  ...
  terminology: {
    ...
    suggested_layer: "template_family",
    automation_posture: "guarded_auto",
    default_summary: "统一摘要或正文中的术语别名、缩写和优选写法。",
    hit_locations: ["摘要术语", "正文术语"],
    sections: ["abstract", "body"],
    table_targets: [],
    not_applicable_when: ["术语替换会改变医学含义时不得自动执行。"],
    human_review_required_when: ["涉及缩写首定义或跨段术语统一时需要人工复核。"],
  },
  heading_hierarchy: {
  ...
  statement: {
    ...
    default_summary: "检查伦理、注册、基金或作者贡献等声明是否完整且落位正确。",
    hit_locations: ["文末声明区"],
    sections: ["back_matter"],
    table_targets: [],
    not_applicable_when: ["稿件类型本身不要求该类声明时不应强制补入。"],
    human_review_required_when: ["声明措辞、缺失原因或落位变更需要人工复核。"],
  },
  manuscript_structure: {
    kind: "manuscript_structure",
    title: "稿件结构包",
    rule_object: "manuscript_structure",
    suggested_layer: "template_family",
    automation_posture: "inspect_only",
    default_summary: "检查一级章节是否完整，并符合目标稿件类型的结构顺序。",
    hit_locations: ["章节提纲"],
    sections: ["body"],
    table_targets: [],
    not_applicable_when: ["自由体例或评论类文章不遵循固定结构时不应强制套用。"],
    human_review_required_when: ["新增、删减或重排一级章节时需要人工复核。"],
  },
  numeric_statistics: {
...
export const RULE_PACKAGE_RECOGNIZERS: RulePackageRecognizer[] = [
  ...
  (input) => recognizePackage("terminology", input),
  (input) => recognizePackage("heading_hierarchy", input),
  (input) => recognizePackage("statement", input),
  (input) => recognizePackage("manuscript_structure", input),
