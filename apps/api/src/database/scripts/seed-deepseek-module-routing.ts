...
const MANAGED_PROVIDER_KIND = "deepseek";
const appRoot = path.resolve(import.meta.dirname, "../../..");

export interface EnsureDeepSeekModuleRoutingConfigurationInput {
  actorRole: RoleKey;
  actorId?: string;
...
export interface EnsureDeepSeekModuleRoutingConfigurationResult {
  connection: {
    id: string;
...
export async function ensureDeepSeekModuleRoutingConfiguration(
  input: EnsureDeepSeekModuleRoutingConfigurationInput,
): Promise<EnsureDeepSeekModuleRoutingConfigurationResult> {
  const connection = await ensureDeepSeekConnection(input);
...
async function ensureDeepSeekConnection(
  input: EnsureDeepSeekModuleRoutingConfigurationInput,
): Promise<{
  ...
  const desiredBaseUrl = normalizeRequiredString(
    input.connection.baseUrl ?? DEFAULT_DEEPSEEK_BASE_URL,
    "connection.baseUrl",
  );
...
async function ensureModuleDefaults(input: {
  ...
  >;
  modules: EnsureDeepSeekModuleRoutingConfigurationInput["modules"];
  ensuredModels: Array<{
    key: string;
    model: ModelRegistryRecord;
    status: "created" | "updated" | "unchanged";
...
function readModuleConfigFromEnv(
  ...
  moduleKey: SystemSettingsModuleKey,
): EnsureDeepSeekModuleRoutingConfigurationInput["modules"][SystemSettingsModuleKey] {
  const prefix = `DEEPSEEK_${moduleKey.toUpperCase()}`;
  return {
...
function readCliConfigFromEnv(
  ...
  EnsureDeepSeekModuleRoutingConfigurationInput,
  | "actorRole"
  | "actorId"
  | "aiProviderConnectionService"
  | "modelRegistryService"
...
> {
  return {
    connection: {
      name: env.DEEPSEEK_CONNECTION_NAME?.trim() || "DeepSeek Production",
      baseUrl: env.DEEPSEEK_BASE_URL?.trim() || DEFAULT_DEEPSEEK_BASE_URL,
      apiKey: normalizeRequiredString(env.DEEPSEEK_API_KEY, "DEEPSEEK_API_KEY"),
      enabled:
        env.DEEPSEEK_CONNECTION_ENABLED === undefined
          ? true
          : ["1", "true", "yes", "on"].includes(
              env.DEEPSEEK_CONNECTION_ENABLED.trim().toLowerCase(),
            ),
    },
    modules: {
...
export async function runSeedDeepSeekModuleRoutingCli(
  ...
  try {
    ...
    const result = await ensureDeepSeekModuleRoutingConfiguration({
      actorRole: "admin",
      actorId: "system-cli",
...
if (isDirectExecution()) {
  runSeedDeepSeekModuleRoutingCli().catch((error) => {
    console.error(error);
    process.exit(1);
  });
}
