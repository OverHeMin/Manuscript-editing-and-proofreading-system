...
- `apps/web` 里的 admin-console 现在还可以把 Recent Agent Executions 直接下钻到 manuscript、job 与 created asset 输出明细，方便治理台追踪运行结果
...
- `pnpm verify:manuscript-workbench` 当前已经覆盖 manuscript handoff、learning review flow、knowledge review handoff + approve/reject terminal actions、admin governance execution observability、evaluation workbench suite activation + manual evaluation loop 的真实浏览器门禁，并包含 verification-ops 持久化 HTTP 路由回归
