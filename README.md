...
4. 在启动 persistent API 前执行共享 preflight
  ...
  - `Phase 10U` stabilizes the same local-first JSON lane: `--json` output now keeps the existing report fields intact while adding additive top-level metadata such as `report_kind`, `contract_version`, and `requested_options` for machine consumers.
  ...
  - `Phase 10W` extends the same local recovery lane for human operators: after a successful replay pass, the command now prints one read-only residual summary for the same scoped backlog. This additive observation ignores replay budget narrowing, stays fail-open, and intentionally leaves `--json` output unchanged.
5. 启动本地 demo API
   `pnpm --filter @medical/api run serve:demo`
...
- `Phase 10W` extends the same manual recovery lane with one post-pass residual summary in human replay output, so operators can see what remains in the same scope without running a second command. This stays read-only, fail-open, and does not widen the existing `--json` contract.
