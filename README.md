...
- Evaluation Workbench 已可加载 verification-ops 的 suites / runs / run items，并支持在浏览器中激活 draft suite、创建 evaluation run、录入 run item 结果、finalize evidence/recommendation、以及发起 learning handoff
...
- `apps/web` 里的 evaluation-workbench 已不再是占位页，当前可以直接查看 verification-ops 的 suite/run/run-item 概览，并在浏览器中完成 run 创建、run-item 录入、evidence finalize、learning candidate handoff
- `pnpm verify:manuscript-workbench` 当前已经覆盖 manuscript handoff、learning review flow、knowledge review handoff + approve/reject terminal actions、evaluation workbench suite activation + manual evaluation loop 的真实浏览器门禁，并包含 verification-ops 持久化 HTTP 路由回归
